<template>
  <view class="gui-swiper-card-wrap">
    <swiper
      ...
      @change="swiperchange"
    >
      <swiper-item v-for="(item, index) in swiperItems" :key="index" class="gui-swiper-card-item gui-border-box">
        <navigator
    ...
    >
      <view class="gui-indicator-dots-wrap gui-flex gui-row gui-nowrap gui-justify-content-center">
        <view
          v-for="(item, index) in swiperItems"
          :key="index"
          :class="['gui-indicator-dot', current == index ? 'dot-show' : '', current == index ? indicatorActiveClass : indicatorClass]"
          :style="{
            ...
            backgroundColor: current != index ? 'rgba(255, 255, 255, 0.6)!important' : 'rgba(255, 255, 255, 1)!important',
          }"
        ></view>
      </view>
    </view>
...
export default {
  ...
  props: {
    ...
    indicatorActiveWidth: { type: Number, default: 24 },
...
.dot-show {
  animation: dot-show 300ms linear forwards;
}
/* #endif */
</style>
