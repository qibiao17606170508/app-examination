<template>
  <view>
    ...
    <view class="gui-dateBT-shade gui-flex gui-columns gui-justify-content-end" v-if="show" :style="{ zIndex: zIndex, width: width }">
      <view class="graceDateTime-header gui-bg-gray gui-dark-bg-level-1 gui-flex gui-row gui-space-between">
        <text class="graceDateTime-header-btn gui-color-gray" @tap="close">{{ cancelText }}</text>
        <text class="graceDateTime-header-btn gui-primary-text" style="text-align: right" @tap="confirm">{{ confirmText }}</text>
      </view>
    ...
    changeBase: function (res) {
      ...
      if (this.isTime) {
        ...
        if (this.isDay) {
          ...
          var resdata = new Array(this.sDate[0][this.defaultVal[0]], this.sDate[1][this.defaultVal[1]]);
        }
      }
