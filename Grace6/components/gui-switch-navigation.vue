<template>
  <scroll-view :scroll-with-animation="scorllAnimation" :scroll-x="true" :show-scrollbar="false" :class="['gui-scroll-x', isCenter ? 'gui-nav-center' : '']" :style="{ width: width + 'rpx' }" :scroll-into-view="autoLeft" :scroll-left="scrollLeft">
    <view
      ...
      :data-index="index"
    >
      <view class="gui-flex gui-nowrap gui-align-items-start" :class="[textAlign == 'center' ? 'gui-justify-content-center' : '']">
        <text
        ...
        <view v-if="item.tips">
          <text v-if="item.tips != 'point'" class="gui-nav-tips gui-block" :style="tipsStyle">{{ item.tips }}</text>
          <text v-else class="gui-nav-tips gui-block" :style="tipsStyle + '; width:12rpx; height:12rpx; over-flow:hidden; padding:0rpx; margin-top:10rpx;'"></text>
        </view>
      </view>
