<template>
  <view :style="{ opacity: pageStatus ? 1 : 0 }" class="gui-sbody gui-flex gui-column" :class="[fullPage || refresh || loadmore ? 'gui-flex1' : '']">
    <!-- 自定义头部 -->
    <view class="gui-header gui-transition-all" :class="headerClass" :style="headerStyle" v-if="customHeader" id="guiPageHeader" ref="guiPageHeader">
      <!-- 状态栏 -->
    ...
    <view class="gui-flex1 gui-relative" v-if="refresh || loadmore" id="guiPageBody" ref="guiPageBody" :style="{ marginTop: fixedTopMargin + 'px' }">
      <scroll-view class="gui-absolute-full" :scroll-y="true" :show-scrollbar="false" @touchstart="touchstart" @touchmove="touchmove" @touchend="touchend" @scroll="scroll" :scroll-into-view="topTagID" :scroll-with-animation="false" @scrolltolower="loadmorefun">
        <view id="guiPageBodyTopTag">
          <gui-refresh ref="guiPageRefresh" @reload="reload" :refreshText="refreshText" :customClass="refreshClasses" :triggerHeight="reFreshTriggerHeight" :refreshFontSize="refreshFontSize"></gui-refresh>
        </view>
        <slot name="gBody"></slot>
        <view v-if="loadmore">
          <gui-loadmore ref="guipageloadmore" :status="loadMoreStatus" :loadMoreText="loadMoreText" :customClass="loadMoreClass" :loadMoreFontSize="loadMoreFontSize"></gui-loadmore>
        </view>
      </scroll-view>
    </view>
    <!-- #endif -->
    ...
    <view class="gui-flex gui-column gui-flex1" v-if="refresh || loadmore" id="guiPageBody" ref="guiPageBody" :style="{ marginTop: fixedTopMargin + 'px' }">
      <list :show-scrollbar="false" class="gui-flex1" @loadmore="loadmorefun" @scroll="scroll">
        ...
        <cell v-if="refresh">
          <gui-refresh ref="guiPageRefresh" :refreshText="refreshText" :customClass="refreshClasses" :triggerHeight="reFreshTriggerHeight" :refreshFontSize="refreshFontSize"></gui-refresh>
        </cell>
        <slot name="gBody"></slot>
    ...
    <!-- 吸顶元素 -->
    <view class="gui-page-fixed-top" ref="guiPageFixedTop" id="guiPageFixedTop" :style="{ top: fixedTop + 'px' }">
