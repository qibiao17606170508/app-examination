<template>
  <div class="sidebar-subfield-container flex-box">
    <div class="side flex-box flex_d-column">
      ...
      <el-scrollbar class="flex-item_f-1">
        <div class="piece-group flex-box flex_d-column flex_a_i-center">
          ...
          >
            <Iconfont v-if="item.icon" size="20px" :name="item.icon" />
            <div class="width-full text-align-center ellipse">
              <span>
                {{ currentLanguage == 'en' ? item.nameEn : item.nameCn }}
              </span>
            </div>
          </div>
        </div>
    ...
    <div
      class="menu flex-box flex_d-column"
      v-show="active.children && active.children.length"
    >
      <el-scrollbar class="flex-item_f-1">
        <el-menu
...
export default defineComponent({
  ...
  setup() {
    ...
    const menus = computed<MenuItem[]>(() => store.getters['menu/menus'])
     ...
     * @param {MenuItem} row
     */
    const clickHandle = (row: MenuItem) => {
      active.value = row
      switch (row.type) {
        case 4: // 外链
          window.open(row.url)
     ...
     * 递归查找当前菜单
     */
    const findMenu = (list: MenuItem[]): MenuItem | null => {
     ...
     * @param {MenuItem[]} list
     */
    const findRootMenu = (list: MenuItem[]): MenuItem | undefined => {
      for (let i = 0; i < list.length; i++) {
        const item = list[i]
        if (item.name === route.name) {
          return item
...
.sidebar-subfield-container {
  ...
  /* max-width: var(--gl-sidebar-subfield-width) !important; */
  .side {
    max-width: var(--gl-sidebar-subfield-side-width);
    z-index: 10;
